timescale_sens <- function(df) {
  ...
  imp <- mice(df, m = 1, predictorMatrix = predmat, methods = imp_methods)
  imp <- complete(imp)
  ...
  out <- out |>
    ...
    mutate(
      Shift = ifelse(Shift == -1, "Add 1 hr sleep", "Remove 1 hr sleep")
    ) |>
  ...
  ggsave(
    file.path(data_dir, "stratified_hrs.png"),
    device = "png",
    bg = "white",
    width = 6,
    height = 8
