...
# histograms 
d |> 
  select(BMI, num_non_cancer_illness, townsend_deprivation_index, sleep_duration_sr,
...
levels(d$chronotype) <- c("prefer not answer", "dont know", "morning",
...
levels(d$freq_depressed_twoweeks) <- c("prefer not answer", "dont know",
                                          "not at all", "several days","more than half",
                                          "nearly every day")

d$freq_depressed_twoweeks <- fct_relevel(d$freq_depressed_twoweeks,
                                            "not at all") # not at all as ref category
...
levels(d$avg_total_household_income) <- c("prefer not answer", "dont know",
                                             "<18", "18-30", "31-50", "52-100",">100")

d$avg_total_household_income <- fct_relevel(d$avg_total_household_income, 
                                               "31-50") # 31-50 as ref category
...
levels(d$employment) <- c("none of above", "prefer not answer",
...
nd <- expand_grid(weekday = levels(d3$weekday),
                  accel_daylight_savings_crossover = 0,
                  eid = unique(d3$eid))

nd$pred <- as_tibble(predict(m1, newdata = nd))$value

nd <- nd |> group_by(eid) |> summarise(avg_sleep = mean(pred))
...
nd2 <- expand_grid(weekday = levels(d3$weekday),
                  accel_daylight_savings_crossover = 0,
                  eid = unique(d3$eid))

nd2$pred <- as_tibble(predict(m2, newdata = nd2))$value

nd2 <- nd2 |> group_by(eid) |> summarise(avg_inactivity = mean(pred))
...
nd3 <- expand_grid(weekday = levels(d3$weekday),
                   accel_daylight_savings_crossover = 0,
                   eid = unique(d3$eid))

nd3$pred <- as_tibble(predict(m3, newdata = nd3))$value

nd3 <- nd3 |> group_by(eid) |> summarise(avg_light = mean(pred))
...
nd4 <- expand_grid(weekday = levels(d3$weekday),
                   accel_daylight_savings_crossover = 0,
                   eid = unique(d3$eid))

nd4$pred <- as_tibble(predict(m4, newdata = nd4))$value

nd4 <- nd4 |> group_by(eid) |> summarise(avg_mvpa = mean(pred))
...
demtime <- d |> 
  mutate(date_all_cause_dementia = as_date(date_all_cause_dementia),
...
d2 <- d2 |> 
  mutate(source_all_cause_dementia_report = 
