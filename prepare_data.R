...
dem_df$insomnia_dx <- ifelse(
  dem_df$insomnia_sr == 1 |
    dem_df$date_insomnia_dx <= dem_df$calendar_date, 1, 0
)
dem_df$sleep_disorder_dx <- ifelse(
  dem_df$sleep_disorder_sr == 1 |
    dem_df$date_any_sleep_dx <= dem_df$calendar_date, 1, 0
)
...
dem_df$freq_depressed_twoweeks <- fct_relevel(
  dem_df$freq_depressed_twoweeks, "not at all"
)
...
levels(dem_df$overall_health_rating) <-
  c("prefer not answer", "dont know", "excellent", "good", "fair", "poor")
dem_df$shift <- ifelse(
  dem_df$job_night_shift %in% c(3, 4) |
    dem_df$job_shift_work %in% c(3, 4), 1, 0
)
...
dem_df$age_accel <-
  dem_df$age_assessment +
  ((as.Date(dem_df$calendar_date) - as.Date(dem_df$date_baseline)) / 365)
...
dem_df <- dem_df |>
  mutate(competing = ifelse(
    !is.na(date_of_death) & is.na(date_acdem2), 1, 0
  )) |>
...
prev <- prev |> mutate(across(starts_with("sr_"), ~ ifelse(
  is.na(.), 0, .
)))
