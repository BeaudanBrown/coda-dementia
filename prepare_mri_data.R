...
mri <- fread(file.path(
  data_dir, "../MRI/mri_full_trimmed.csv"
))
...
mri <- mri |> select(
  eid, age_assessment_mri1, date_mri1, assessment_centre_mri1,
  height_mri1, head_scale, tot_wmh, tbv, wmv, gmv, hip, log_wmh
)
...
d <- fread(file.path(data_dir, "24hr_behaviours_25_03_24.csv"),
  stringsAsFactors = T
)
...
fwrite(mri_model_data, file.path(data_dir, "mri_data.csv"))
